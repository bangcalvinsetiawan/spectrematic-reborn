var AppId = "7388c5278c4340402f35d9006e607f68";
...
if (getCookie("_authToken")) {
  $("#txtToken").val(getCookie("_authToken"));
}
...
/*hisTory.addEventListener('click', function(e) {
    if(!$(this).hasClass('active')){
      $("#open_orders_his").html('<div style=" "><img src="./media/loading.gif"/>    </div>')
      var mss = {
          action: "userhistory",
          data: {
            account_type:$("#cmbAccountType option").filter(":selected").val(),
      ...
      fsocket.send(JSON.stringify(mss));
    }
  },false);
  */
  ...
  switch (js.action) {
    case "userdata":
      if (js.userdata.hasOwnProperty("user_id")) {
        ...
        setCookie("_authToken", $("#txtToken").val(), "365");
      ...
      if (Demo) {
        ...
        if (match == null) {
          ...
          if (trx_id == transaction_id) {
            ...
            var plD = parseFloat($("#lblCurrentBalanc").text()) - parseFloat($("#lblStartBalance").text());
            ...
            if (parseFloat(profit) == 0) {
              ...
              if (coi > 0x32) tableS = tableS.slice(0, -tableL[coi - 0x32]);
              OnTrade = false;
              //last_investment = 1;
        ...
        if (match == null) {
          ...
          if (trx_id == transaction_id) {
            ...
            var pl = parseFloat($("#lblCurrentBalance").text()) - parseFloat($("#lblStartBalance").text());
            $("#lblPL")
              .text(pl.toFixed(3))
            ...
            if (parseFloat(profit) == 0) {
              ...
              $("#lblTotalWin").text(parseFloat($("#lblTotalWin").text()) + 1);
            } else if (parseFloat(profit) > 0) {
              $("#lblTradeStatus").text("WIN");
              total++;
              Bg = "#77f2bd";
              ...
              tableL[coi] = table.length;
              if (coi > 0x32) tableS = tableS.slice(0, -tableL[coi - 0x32]);
              //if ($("#lossvirtual").val() != 0) Demo = true;
              // lost_t = 0;
              // win_t++;
              // alert(trx);
              if ($("#customSwitch2").is(":checked") == true) {
            ...
            if (parseFloat($("#txt_takeprofit").val()) > 0) {
              ...
              if (parseFloat($("#lblPL").text()) > 0 && parseFloat($("#lblPL").text()) >= parseFloat($("#txt_takeprofit").val())) {
                $("#btnStart").text("Start");
                // alert('Horrey... your take profit');
                Swal.fire({
                  icon: "success",
                  title: "Notification",
                  html: "Horrey... your take profit <b style='color: green'>" + plt + "</b> has been reached !",
                }).then(function () {
                  // location.reload();
                });
              }
            ...
            if (parseFloat($("#txt_stoploss").val()) > 0) {
              if (parseFloat($("#lblPL").text()) < 0 && Math.abs(parseFloat($("#lblPL").text())) >= parseFloat($("#txt_stoploss").val())) {
                $("#btnStart").text("Start");
                // alert('Oops... Stoploss')
                Swal.fire({
                  icon: "warning",
    ...
    /*case "userhistory":
              if (js.action == 'userhistory') {
                  $("#loader_expired_trades").remove();
                  if (js.userhistory == null || js.status == 'error') {
                  // alert('1')
                      $("#open_orders_his").append("<span style='width:100%;text-align:center'>You currently have no  trades.</span>")
                  } else {
                      trade_data = js.userhistory;
                      // $("#open_orders_his").html(template)
                      template = '';
                      reached_end=0;
                      for (zz = 0; zz < trade_data.length; zz++) {
                          tr=zz+1;
                          transaction_id = trade_data[zz].transaction_id;
                          asset_name = trade_data[zz].asset;
                          direction = trade_data[zz].direction;
                          entry_price = trade_data[zz].entry_price;
                          spot_price = trade_data[zz].exit_price;
                          duration = trade_data[zz].expiration_text;
                          protection = trade_data[zz].cap_protection;
                          exit_time = trade_data[zz].expiry_time;
                          entry_time = trade_data[zz].entry_time;
                          investment = trade_data[zz].investment;
                          payout = trade_data[zz].profit;
                          profi = trade_data[zz].profit;
                          transaction_outcome = trade_data[zz].transaction_outcome;
                          ...
                          if (direction == 'CALL') {
                              ...
                              direction_txt = 'UP';
                              if (transaction_outcome == 1) {
                                  payout_color = '#17b04c';
                              }
                              if (transaction_outcome == 0) {
                                  payout_color = '#c33332';
                                  payout = investment * (protection / 100).toFixed(3);
                              }
                              if (transaction_outcome == 3) {
                                  payout_color = 'white';
                              }
                          } else {
                          ...
                          // if (btnPu.addEventListener('click', function(e) )){
                              // if(js.action == 'userhistory') {
                          if (trade_data !=undefined){
                          // $("#open_orders_his").html("<span style='width:100%;text-align:center'>You currently have no  trades.</span>")
                              template += "<tr style='background:"+payout_color+";color:"+color+"'><td>"+tr+"</td><td>"+transaction_id+"</td><td> "+asset_name+" </td> <td>"+direction_txt+"</td><td> "+duration+"</td><td>"+investment+"</td> <td>"+entry_price+"</td> <td>"+spot_price+"</td><td>"+Sta+"</div><td>"+profi+"</td></tr>";
                          // }else{
                          //     reached_end=1;
                          // }
