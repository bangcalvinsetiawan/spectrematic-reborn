...
//token = $("#txt_t").val();
//if(window.open("")){
 // window.localStorage.setItem("access_token", "<?php print $access_token; ?>");
...
//window.setCookie("_authToken", "<?php print $access_token; ?>", "365");
if (getCookie("_authToken")) {
  $("#txt_t").val(getCookie("_authToken"));
}
//load last account type a5a88308819e1b47aaa4e810af906bc38dd35074
...
cmb_signal.addEventListener(
  ...
  function (e) {
    ...
    if ($("#cmb_signal option").filter(":selected").val() === "Xspott") {
        $("#Ticpoin").hide();
        $("#Limit").show();
    };
    ...
    case "TradeDetails":
      ...
      case "tradesubscribe":
       //console.log(js);
            ...
            } else {console.log(js.msg.transaction_id);
                lossvirtual = 0;
                var regex1 = /Outcome/g;
                var match = event.data.match(regex1);
                if (match == null) {
